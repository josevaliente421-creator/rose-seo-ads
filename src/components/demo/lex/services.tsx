          ...
          {services.map((service, i) => (
            <Reveal key={service.title} delay={0.05 * (i % 3)}>
              <article className="group relative h-full overflow-hidden rounded-2xl border border-white/[0.08] bg-white/[0.02] p-7 transition-all duration-500 hover:-translate-y-1 hover:border-[var(--demo-gold)]/35 hover:bg-white/[0.04]">
                <div
                  aria-hidden
                  className="absolute -right-10 -top-10 size-32 rounded-full bg-[var(--demo-burgundy)]/30 blur-2xl opacity-0 transition-opacity duration-500 group-hover:opacity-100"
                />
                ...
                <h3 className="relative mt-6 font-demo-serif text-2xl font-semibold tracking-tight text-[#f6f1ea]">
                  {service.title}
                </h3>
