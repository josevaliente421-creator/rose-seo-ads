          ...
          {team.map((member, i) => (
            <Reveal key={member.name} delay={0.06 * i}>
              <article className="group h-full rounded-2xl border border-white/[0.08] bg-white/[0.02] p-7 transition-all duration-500 hover:-translate-y-1 hover:border-[var(--demo-gold)]/35">
                <div
                  aria-hidden
                  className="flex size-16 items-center justify-center rounded-full bg-gradient-to-br from-[var(--demo-burgundy)] to-[#12060b] font-demo-serif text-xl font-semibold text-[var(--demo-gold)] ring-1 ring-[var(--demo-gold)]/30 transition-all duration-500 group-hover:ring-[var(--demo-gold)]/70"
                >
                  {member.initials}
                </div>
                <h3 className="mt-6 font-demo-serif text-xl font-semibold tracking-tight text-[#f6f1ea]">
                  {member.name}
                </h3>
